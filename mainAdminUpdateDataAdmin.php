...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $btnFillData) {
    ...
    $ID = $_POST["ID"];
    	...
    	
      {
        ...
        $sql = "SELECT ID FROM admin WHERE ID='$url_id'";
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST" && $btnUpdate) {
      ...
      if(strcmp($password, $checkPassword) != 0) {
        ...
        $sqlUpdateAdmin="UPDATE admin SET fName = '$_POST[fName]', lName = '$_POST[lName]', 
        ...
        $update2 = mysqli_query($con1, $sqlUpdateAdmin);
        if(mysqli_affected_rows($con1) == 0) {
            $zero2 = true;
        }
        if(mysqli_affected_rows($con1) >= 0) {
...
<div align = "center">
  ...
  <h3 style = "margin-top: 7.0vw;">Актуализиране данни на подадминистратор.</h3>
  <br>
  ...
  <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" accept-charset="UTF-8" style = "">  
	...
	<table class = "iDataInput">
        ...
        <tr>
	        ...
	        <td><br>Парола*<br><input type="password" name="password" value = "<?php echo $password; ?>" required="required" placeholder = "Задължително попълване"></td>
