    ...
    */	  
    	...
    	if(!mysqli_num_rows($result1) >0){
    		...
    		if($row = mysqli_fetch_array($result))
            {
                ...
                $driverPhoneNumber = $row['Driver_Phone_Number'];
                //$tp = $row['TP'];
                //$go = $row['GO']; 
                //$gtp = $row['GTP'];
                //$kasko = $row['Kasko'];
...
//	  }
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST" && $btnUpdate) {
        ...
        if(strlen($_POST['Driver_First_Name']) == 0) {
...
//    }  
            ...
            if (mysqli_num_rows($sqlShowUpdatedData) > 0) {
	          ...
	          echo "<table border='2' style = 'margin-top: 6.0vw;'>
	          ...
	          <th bgcolor='$color1'>$h2 Стойност на МПС лв</th>
         ...
         $sqlUpdateAutos="UPDATE autos SET  
            ...
            Transmission = '$_POST[Transmission]', Guarantee = '$_POST[Guarantee]', GuaranteeDate = '$_POST[GuaranteeDate]',
            Price = '$_POST[Price]', Address_MPS = '$_POST[Address_MPS]', Current_Km = '$_POST[Current_Km]', Date_Current_Km = '$_POST[Date_Current_Km]', 
            Driver_First_Name = '$_POST[Driver_First_Name]', Driver_Last_Name = '$_POST[Driver_Last_Name]', Driver_Phone_Number = '$_POST[Driver_Phone_Number]', 
            TP = '$_POST[TP]', GO = '$_POST[GO]', GTP = '$_POST[GTP]', Kasko = '$_POST[Kasko]',
            Vinetka = '$_POST[Vinetka]', Others = '$_POST[Others]', MAT = '$_POST[MAT]'
            Where AutosID = '$_POST[AutosID]'";
            ...
            if ($update && mysqli_affected_rows($con) == 0) {
             mysqli_commit($con);
	          ...
	          <td>$row[Current_Km]</td>
              <td>$row[Date_Current_Km]</td>
              <td>$row[Driver_First_Name]</td>
              <td>$row[Driver_Last_Name]</td>
              <td>$row[Driver_Phone_Number]</td>
              <td>$row[TP]</td>
              <td>$row[GO]</td>
              <td>$row[GTP]</td>
              <td>$row[Kasko]</td>
...
//    }  
            ...
            if (mysqli_num_rows($sqlShowUpdatedData) > 0) {
	          ...
	          echo "<td style='color:black;'>" . $row['Current_Km'] . "</td>";
              echo "<td style='color:black;'>" . $row['Date_Current_Km'] . "</td>";
              echo "<td style='color:black;'>" . $row['Driver_First_Name'] . "</td>";
              echo "<td style='color:black;'>" . $row['Driver_Last_Name'] . "</td>";
              echo "<td style='color:black;'>" . $row['Driver_Phone_Number'] . "</td>";
              echo "<td style='color:black;'>" . $row['TP'] . "</td>";
              echo "<td style='color:black;'>" . $row['GO'] . "</td>";
              echo "<td style='color:black;'>" . $row['GTP'] . "</td>";
              echo "<td style='color:black;'>" . $row['Kasko'] . "</td>";
...
<script>
    ...
    function toggleEnable() {
        ...
        var driverValue = document.getElementById('driverfName').value;
        
        if(driverValue.localeCompare('') == 0) {
            document.getElementById('driverfName').disabled = true;
            document.getElementById('driverlName').disabled = true;
            document.getElementById('driverPhone').disabled = true;
        }
        //else if(legalValue.localeCompare('') != 0){
        else {    
            document.getElementById('driverfName').disabled = false;
            document.getElementById('driverlName').disabled = false;
            document.getElementById('driverPhone').disabled = false;
        }
...
<div align = "center">
  ...
  <h3 style = "margin-top: 6.0vw;">Актуализиране общи данни на МПС:</h3>
  <br>
  ...
  <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" accept-charset="UTF-8">  
    <table class = "iDataInput">
        ...
        <tr>
	        ...
	        <td>Тегло*<br><input type="number" value="<?php echo $Weight;?>" name="Weight" required="required" placeholder = "Задължително попълване"></td>
        ...
        <tr>
	        ...
	        <td>Дом адрес на МПС*<br><input type="text" value='<?php echo $Address_MPS;?>' name="Address_MPS" required="required" placeholder = "Задължително попълване"></td>
        </tr>
        <tr>
            ...
            <td>Дата текущи километри*<br><input  type="date" value='<?php echo $Date_Current_Km;?>' name="Date_Current_Km" required="required" placeholder = "Дата Текущи километри*"></td>
            
            <td>Шофьор име*<br><input  id = "driverfName" type="text" value='<?php echo $driverFirstName;?>' name="Driver_First_Name" required="required" placeholder = "Задължително попълване"></td>
        </tr>
        <tr>    
            <td>Шофьр фамилия*<br><input  id = "driverlName" type="text" value='<?php echo $driverLastName;?>' name="Driver_Last_Name" required="required" placeholder = "Задължително попълване"></td>
	        
	        <td>Шофьор телефонен №*<br><input  id = "driverPhone" type="text" value='<?php echo $driverPhoneNumber;?>' name="Driver_Phone_Number" required="required" placeholder = "Задължително попълване"></td>
	   ...
	   <tr>
        ...
        <tr>
            <td></td>
            <td><input type="submit" name="btnUpdate" value="Актуализиране" style = "border-radius: 2px; color: red;"><br></td>
            <td></td>
        </tr>
    </table>
  </form>
