...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $btnFillData) {
    ...
    $Legalentity_ID = $_POST["Legalentity_ID"];
    	...
    	
      {
        ...
        $sql = "SELECT Legalentity_ID FROM legalentity WHERE Legalentity_ID='$url_id' AND Sub_Admin_Username = '".$subAdminUsername."'";
        ...
        $result = mysqli_query($con, "SELECT * FROM legalentity
    	                      WHERE Legalentity_ID = '$_POST[Legalentity_ID]' AND Sub_Admin_Username = '".$subAdminUsername."'");
    	if (!$result) {
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST" && $btnUpdate) {
        ...
        if(strlen($_POST['Holding_Name']) == 0) {
            ...
            if(strcmp($password, $checkPassword) != 0) {
                ...
                $sqlUpdateIndividual="UPDATE legalentity SET  
...
<div align = "center">
  ...
  <h3 style = "margin-top: 7.0vw;">Актуализиране данни на юридическо лице:</h3>
  <table class = "iDataInput">
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" accept-charset="UTF-8" style = ""> 
      <tr>
          <td></td>
          <td><p id="demo" style = "margin-bottom: 0px;">ID*</p><input id = "numRec" oninput="checkInput()" type="number" name="Legalentity_ID" value = "<?php echo $Legalentity_ID;?>" required="required" placeholder = "Задължително попълване"></td>
          <td></td>
  	</tr> 
  	<tr>
  	    <td></td>
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" accept-charset="UTF-8" style = "">  
          ...
          <tr>    
              ...
              <!--<td>Потребител*<br><input type="email" name="Sub_Admin_Username" value = "<?php echo $Sub_Admin_Username;?>" readonly></td>-->
              
              <td>Юридическо лице към холдинг<br>
                <input id = "lEntity" type = "text" name="Holding_Name"  value = "<?php echo $Holding_Name;?>" required="required" style = "" readonly></td>
          ...
          <tr>
              ...
              <td>Парола повторно въвеждане*<br><input id = "psw2" type="password" name="checkPassword" value = "<?php echo $Password;?>" required="required" placeholder = "Задължително попълване"></td>
          </tr>
          <tr>
              <td></td>
