...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $btnFillData) {
    ...
    */	  
        ...
        $sql = "SELECT AutosID FROM autos WHERE Reg_Number='$Reg_Number' AND Admin_Username = '$_SESSION[adminUsername]'";
		...
		if(!mysqli_num_rows($resultID) > 0){
    		...
    		//echo"<br><br>";
    	    //echo"<div align = 'center'>";
    	    $message = "Несъществуващ регистрационен номер или нямате достъп до него!";
			...
			$result = mysqli_query($con, "SELECT * FROM insurance
								WHERE AutosID = '$AutosID' AND Admin_Username = '$_SESSION[adminUsername]'");  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			if (!$result) {
...
//	  }
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST" && $btnUpdate) {
      ...
      $con = connectServer();
        ...
        Where AutosID = '$_POST[AutosID]' AND Admin_Username = '$_SESSION[adminUsername]'";
...
<div align = "center">
  ...
  <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" accept-charset="UTF-8" style = ""> 
	...
	<p style = "margin-bottom: 0px;">Рег. №:</p><input type="text" name="Reg_Number" value = "<?php if(isset($_SESSION['Reg_Number_Global'])) echo $_SESSION['Reg_Number_Global']; else echo $Reg_Number;?>" required="required" placeholder = "Задължително попълване">*
	<br><br>
