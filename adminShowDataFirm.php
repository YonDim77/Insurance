...
<div align = "center">;
    <h3 style = "margin-top: 7.0vw;">Данни за юридически лица:</h3>
...
if (($_SERVER["REQUEST_METHOD"] == "POST" || $btnShowData == true) && $btnShowDataAuto == false) {
	...
	$result = mysqli_query($con, "SELECT * FROM legalentity WHERE Name LIKE '$Name%' AND Admin_Username = '".$adminUsername."'"); 
	$resultEik = mysqli_query($con, "SELECT * FROM legalentity WHERE EIK = '$_POST[EIK]' AND Admin_Username = '".$adminUsername."'");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $btnShowDataForm == true) {
	...
	$resultFirm = mysqli_query($con, "SELECT * FROM legalentity WHERE Legalentity_ID = '$Legalentity_ID' AND Admin_Username = '".$adminUsername."'");
	$resultFirmName = mysqli_query($con, "SELECT Name FROM legalentity WHERE Legalentity_ID = '$Legalentity_ID' AND Admin_Username = '".$adminUsername."'");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $btnShowDataAuto == true) {
   ...
   $adminUsername = $_SESSION['adminUsername'];
   $autoId = 0;
   $result = mysqli_query($con, "SELECT * FROM autos WHERE Reg_Number = '$_POST[Reg_Number]' AND Admin_Username = '".$adminUsername."'");
	...
	if (mysqli_num_rows($result) > 0 && $_SESSION['NumberID'] && $_SESSION['Firm']) {
	    ...
	    while($row = mysqli_fetch_array($result))
	    {
	        ...
	        echo "<tr>";
?>	        
	        <td style = "width: 80px;"><button onclick = "location.assign('adminUpdateGeneralDataAuto.php')">Редакция</button></td>
   ...
   $resultTyres = mysqli_query($con, "SELECT * FROM tyres WHERE AutosID = '$autoId'");
	...
	if (mysqli_num_rows($resultInsurance) > 0 && $_SESSION['Firm']) {
	    ...
	    while($row = mysqli_fetch_array($resultInsurance))
	    {
	        echo "<tr>";
?>	        
	        <td style = "width: 80px;"><button onclick = "location.assign('adminUpdateInsuranceDataAuto.php')">Редакция</button></td>
...
<?php
	...
	if (mysqli_num_rows($resultService) > 0 && $_SESSION['Firm']) {
	    ...
	    while($row = mysqli_fetch_array($resultService))
	    {
	        echo "<tr>";
?>	        
	        <td style = "width: 80px;"><button onclick = "location.assign('adminUpdateServiceDataAuto.php')">Редакция</button></td>
...
<?php	        
	...
	if (mysqli_num_rows($resultRepair) > 0 && $_SESSION['Firm']) {
	    ...
	    while($row = mysqli_fetch_array($resultRepair))
	    {
	        echo "<tr>";
?>	        
	        <td style = "width: 80px;"><button onclick = "location.assign('adminUpdateRepairDataAuto.php')">Редакция</button></td>
...
<?php	        
	...
	if (mysqli_num_rows($resultTyres) > 0 && $_SESSION['Firm']) {
	    ...
	    while($row = mysqli_fetch_array($resultTyres))
	    {
	         echo "<tr>";
?>	        
	        <td style = "width: 80px;"><button onclick = "location.assign('adminUpdateTyresDataAuto.php')">Редакция</button></td>
