...
?>
  ...
  <br>
  <h3 style = "">Запис на данни за физическо лице.</h3><br>
  <form name="input" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" accept-charset="UTF-8" style = "">
    <table class = "iDataInput">
        ...
        <tr>
            <td>Имейл на лице за контакти*<br><input type="email" name="Email_Contact_Person" required="required" placeholder = "Задължително попълване"></td>
...
<script>
    function showAutoData() {
        $(document.getElementById('autoData')).toggle(1500);//   style.display = 'block';
    ...
    function hideAutoData() {
        $(document.getElementById('autoData')).toggle(1500);   //style.display = 'none';
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $btnSave == true) {
  ...
  if(strcmp($password, $checkPassword) != 0) {
    ...
    mysqli_query($con,"START TRANSACTION");
     ...
     if (mysqli_query($con, $sqlUsernameCheck) && mysqli_affected_rows($con) == 1 && mysqli_query($con, $sql) && mysqli_affected_rows($con) == 1) {
...
echo "<br><br><br><br>";
if($last_id > 0){
  echo "<table border='2'style = 'margin-bottom: 50px;'>
  
  
  <tr>
  <th bgcolor='$color1'>$h2 ID/№</th>
  <th bgcolor='$color1'>$h2 Потребител<br>Потребителско име</th>
...
?>
  ...
  <form id = "autoData" name="input" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" accept-charset="UTF-8" style = "display: none;">  
    ...
    <table class = "iDataInput">
	   ...
	   <tr>
            ...
            <td></td>
            <td><fieldset id = "mat" >
            МАТ*<br><input type = "text"  name="MAT" value="<?php echo $mat;?>" style = "width:174px; height: 26px;" required="required">
            </fieldset></td>
            <td></td>
  ...
  <button onclick = "location.href='adminInputDataAutoPerson.php'" style = "border-radius: 2px; color: red; width: 27%;">Въвеждане данни за застраховки и сервиз на МПС</button>
  <br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $btnSaveAuto == true) {
  ...
  $sqlInsertAuto = "INSERT INTO autos (Individual_ID, Legalentity_ID, Admin_Username, Sub_Admin_Username, Type, Brand, Model, Reg_Number, Kupe, Shasi, First_Reg, Weight, Color, Seats, Cubature, Power, Engine, 
                                       Transmission, Guarantee, GuaranteeDate, Price, Address_MPS, Current_Km, Date_Current_Km, TP, GO, GTP, Kasko, Vinetka, Others, MAT)
  VALUES
  ('$_POST[Individual_ID]', '$Legalentity_ID', '$_POST[Admin_Username]', '$_POST[Sub_Admin_Username]', '$_POST[Type]', '$_POST[Brand]','$_POST[Model]', '$_POST[RegNumber]', '$_POST[Kupe]', 
  ...
  else
  {
    ...
    $message = "Данните са записани успешно!";
	echo "<script>alert('$message');</script>";
	echo "<script> location.replace('adminInputDataAutoPerson.php'); </script>";
  }
