...
<script>
    function showServiceData() {
        $(document.getElementById('autoServiceData')).toggle(1500);
...
//  $idCheck = mysqli_query($con2, "SELECT * FROM individual WHERE Individual_ID = $_POST[Individual_ID]");
  ...
  if(!$idCheck || mysqli_num_rows($idCheck) < 1 || !mysqli_query($con2, $sqlInsertAuto))
  {
    ...
    $message = "Възникна грешка!Дублиране на ID/№ на МПС, грешно въведено ID/№ на физическо лице!";
  ...
  $resultAutoInsurance = mysqli_query($con2, "SELECT * FROM insurance WHERE Insurance_ID = $lastAutoInsuranceID AND Sub_Admin_Username = '".$subAdminUsername."'");
...
?>
  ...
  <button onclick = "location.href='subAdminInputDataTyresAutoPerson.php'" style = "width: 32%; border-radius: 2px; color: red;">Въвеждане на данни за ремонти и гуми на МПС</button>
  <br><br>
...
//  $idCheck = mysqli_query($con2, "SELECT * FROM individual WHERE Individual_ID = $_POST[Individual_ID]");
  ...
  else
  {
    ...
    $message = "Данните са записани успешно!";
	echo "<script>alert('$message');</script>";
	echo "<script> location.replace('subAdminInputDataTyresAutoPerson.php'); </script>";
  }
  mysqli_query($con2, "SET AUTOCOMMIT=TRUE");
