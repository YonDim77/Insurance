...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $btnFillData) {
    ...
    $ID = $_POST["ID"];
    	...
    	
      {
        ...
        $sql = "SELECT HoldingUsers_ID FROM holdingusers WHERE HoldingUsers_ID='$url_id'";
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST" && $btnUpdate) {
      ...
      if(strcmp($password, $checkPassword) != 0) {
        ...
        $sqlUpdateHoldingUsers="UPDATE holdingusers SET Holding_Name = '$_POST[holdingName]', First_Name = '$_POST[fName]', Last_Name = '$_POST[lName]', 
