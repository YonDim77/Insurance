...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $btnShowData) {
    ...
    $con = connectServer();
    	...
    	$_SESSION['Reg_Number'] = mysqli_real_escape_string($con, $_POST['Reg_Number']);       // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        $sql = "SELECT AutosID FROM autos WHERE Reg_Number='$_SESSION[Reg_Number]' AND Sub_Admin_Username = '$_SESSION[subAdminUsername]'";
    ...
    $con = connectServer();
		...
		$checkForRepairId = mysqli_query($con, "SELECT *FROM repair WHERE Repair_ID = '$Repair_ID' 
		...
		if (!isset($_SESSION['Reg_Number'])){
		    ...
		    $message = "Несъществуващо ID/№ или грешно ID/№ или нямате достъп!";
		    echo "<script>alert('$message');</script>";
		} 
...
//	  }
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST" && $btnUpdate) {
      ...
      $con = connectServer();
        ...
        Where Repair_ID = '$_POST[Repair_ID]' AND Sub_Admin_Username = '$_SESSION[subAdminUsername]'";
...
<div align = "center">
  ...
  <h3 style = "margin-top: 7.0vw;">Актуализиране на данни за ремонт на МПС:</h3>
  <br>
  <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" accept-charset="UTF-8" style = ""> 
	...
	<p style = "margin-bottom: 0px;">Рег. №*</p><input type="text" name="Reg_Number" value = "<?php if(isset($_SESSION['Reg_Number_Global'])) echo $_SESSION['Reg_Number_Global']; else echo $_SESSION['Reg_Number'];?>" style = "width:174px;" required="required" placeholder = "Задължително попълване">
	<br><br>
  ...
  <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" accept-charset="UTF-8" style = "">  
    <table class = "iDataInput">
        <tr>
            ...
            <input type="text" name="Reg_Num" value = "<?php echo $_SESSION['Reg_Number'];?>" style = "display: none;">
	        <br><br>
            <td>Ремонт вид*<br>
