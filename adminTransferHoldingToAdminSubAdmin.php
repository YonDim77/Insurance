...
//if ($_SERVER["REQUEST_METHOD"] == "POST" && $btnFilter) {
    ...
    {
        $showLegalentityData = mysqli_query($con, "SELECT * FROM legalentity Where Holding_Name IS NOT NULL AND Admin_Username = '$_SESSION[adminUsername]'");
    }
...
//}
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST" && $btnUpdate) {
      ...
      $con = connectServer();
		...
		if(strlen($subAdmin) == 0 || count($Legalentity_ID) == 0) {
		   $message = "Моля изберете потребител и маркирайте!";
   	       echo "<script>alert('$message');</script>"; 
		}
		
		else if(strlen($subAdmin) > 0 && count($Legalentity_ID) != $nummberItems) {
		   $message = "Моля изберете потребител и маркирайте всички!";
   	       echo "<script>alert('$message');</script>"; 
		}
	      ...
	      //   }
		    ...
		    SET Admin_Username = '$_SESSION[adminUsername]', Sub_Admin_Username = '$_POST[Sub_Admin_Username]' 
            ...
            if($zeroLegalentity == true && $zeroAutos == true && $zeroInsurance == true && $zeroService == true &&  $zeroRepair == true &&
             ...
             echo "<script>alert('$message');</script>";
             echo "<script> location.replace('adminTransferHoldingToAdminSubAdmin.php'); </script>";  
             //break;                                                                                    
            }
...
<div align = "left">
    ...
    <br><br>
    <select id = "subAdminValue" style = "position: fixed; top: 32.0vw; left: 5.0vw; width: 174px; height: 27px;" name="Sub_Admin_Username" onchange="">
    ...
    <input id = "bUpdate" type="submit" style = "position: fixed; top: 36.0vw; left: 5.0vw;" name="btnUpdate" value="Актуализиране" style = "border-radius: 2px; color: red;">  
  </form>
...
document.getElementById('bUpdate').onclick = function() {
    ...
    if(document.getElementById('subAdminValue').value != "" && checkBox == true)
