...
$resultAutoServiceOFE = mysqli_query($con, "SELECT * FROM service WHERE Service_ID = $_SESSION[lastAutoServiceFirmID]
                                            AND Type = 'масла и филтри'");

$resultAutoService = mysqli_query($con, "SELECT * FROM service WHERE Service_ID = $_SESSION[lastAutoServiceFirmID]");

if($_SESSION['lastAutoServiceFirmID'] > 0) {
   
  echo "<h3 style = 'margin-top: 6.0vw';>Последно въведени данни за сервиз на МПС на юридическо лице</h3>";
    
  if(mysqli_num_rows($resultAutoServiceOFE) < 1) {
    
    echo "<table border='2' style = 'margin-top: 3.0vw';>
...
<h3 style = "margin-top: 6.0vw;">Запис на данни за ремонти на МПС на юридическо лице:</h3>
  <form name="input" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" accept-charset="UTF-8" style = "margin-top: -30px;">  
    <table class = "iDataInput">
        <tr>    
            ...
            <td>Подадмин потребителско име*<br>
    ...
    <br><br>
    <input type="submit" name="btnSaveRepairAuto" value="Запис" style = "width: 32%; border-radius: 2px; color: red;">  
  </form>
  <br><br>
  
  <button id = "showTyresData" onclick = "showTyresData()" style = "border-radius: 2px; color: red; width: 32%;">Покажи запис на данни за гуми на МПС на юридическо лице</button>
                <button id = "hideTyresData" onclick = "hideTyresData()" style = "border-radius: 2px; color: red; width: 32%; display: none;">Скрий запис на данни за гуми на МПС на юридическо лице</button>
                <br><br>
<script>
...
//  $idCheck = mysqli_query($con2, "SELECT * FROM individual WHERE Individual_ID = $_POST[Individual_ID]");
  ...
  $idCheck = mysqli_query($con2, "SELECT * FROM service WHERE AutosID = $_POST[AutosID] 
...
function showSubAdmin1(str) {
  ...
  <form id = "tyresData" name="input" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" accept-charset="UTF-8" style = "display: none;">  
    <h3>Запис на данни за гуми на МПС на юридическо лице:</h3>
    <br><br>
    <table class = "iDataInput">
            <!--<input type="text" name="Admin_Username" value = "<?php //echo $_SESSION['username']; ?>" style = "display: none;">-->
        <tr>
        ...
        <tr>
            ...
            <br><br>
    <input type="submit" name="btnSaveTyresAuto" value="Запис" style = "width: 32%; border-radius: 2px; color: red;">  
  </form>
  <br><br>
...
//  $idCheck = mysqli_query($con2, "SELECT * FROM individual WHERE Individual_ID = $_POST[Individual_ID]");
  ...
  $idCheck = mysqli_query($con2, "SELECT * FROM repair WHERE AutosID = $_POST[AutosID] 
                                  AND Legalentity_ID = $_POST[Legalentity_ID]
                                  AND Admin_Username = '$_POST[Admin_Username]'
                                  AND Sub_Admin_Username = '$_POST[Sub_Admin_Username]'");
