...
<div align = "center">;
    <h3 style = "margin-top: 6.0vw;">Запис на документи на МПС на юридически лица:</h3>
    <br>
      <form name="input" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" accept-charset="UTF-8">  
        ...
        Име:<br><input style = "" type="text" name="Name" value = "<?php echo $_SESSION['Name']; ?>" placeholder = "Попълнете едното "/>
        <br><br>
...
if (($_SERVER["REQUEST_METHOD"] == "POST" || $btnShowData == true) && $btnShowDataAuto == false && $btnUploadDataAuto == false) {
	...
	$result = mysqli_query($con, "SELECT * FROM legalentity WHERE Name LIKE '$Name%' AND Sub_Admin_Username = '$_SESSION[subAdminUsername]'"); 
	$resultEik = mysqli_query($con, "SELECT * FROM legalentity WHERE EIK = '$_POST[EIK]' AND Sub_Admin_Username = '$_SESSION[subAdminUsername]'");
	...
	{				  
	    ...
	    if (mysqli_num_rows($result) > 0) {
            ...
            echo '<span id = "text" style="font-size: 20px; color:black;"><br><br>Данни за фирма с име или име започващо с: </span>' . "  " .
	        ...
	        while($row = mysqli_fetch_array($result))
	        {
	            ...
	            echo "<td>"; echo'<input style = "width: 50px; border: 0px; text-align: center;" type = "text" value="'. $row['Legalentity_ID']. '" name = "Legalentity_ID' . $index . '" readonly>'; echo"</td>";
	    ...
	    if (mysqli_num_rows($resultEik) < 1) {
	    	...
	    	$message = "Няма фирма с ЕИК: " . $_SESSION['Eik'] . " в базата данни!";
	    	echo "<script>alert('$message');</script>";
	    }
	    ...
	    echo"<div align = 'center'>";
        echo '<span id = "iFace" style="font-size: 20px; color:black;"><br><br>Данни за фирма с ЕИК: </span>' . "  " .
...
?>
	...
	<form method="post" enctype="multipart/form-data">
	    ...
	    <input type="number" value = "<?php echo $legalentityID;?>" name="Legalentity_ID" required="required" style = "display: none">
    ...
    if($_FILES['pdf']['size'] > 0)
    {
        ...
        if($_FILES['pdf']['size'] > 1048576) { //1 MB (size is also in bytes)
            ...
            {  
                 ...
                 { 
                      echo "<br><br>"; 
                      echo '<span style="font-size: 25px; color:black;">Документи на МПС с регистрационен номер ' . '</span>' . '<span style = "font-size: 25px; color: red;">' . $_POST['Reg_Number'] . '</span>';
                      echo "<br><br>";
                      echo '  
                           <tr>  
                                <td>  
            ...
            {
                echo '<script>alert("Възникна грешка или вече има документи записани за този регистрационен номер!")</script>';
            }
        }
    }
