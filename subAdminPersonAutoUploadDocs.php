...
if (($_SERVER["REQUEST_METHOD"] == "POST" || $btnShowData == true) && $btnShowDataAuto == false && $btnUploadDataAuto == false) {
	...
	$result = mysqli_query($con, "SELECT * FROM individual WHERE Names LIKE '$Names%' AND Sub_Admin_Username = '$_SESSION[subAdminUsername]'"); 
	$resultEgn = mysqli_query($con, "SELECT * FROM individual WHERE EGN = '$_POST[EGN]' AND Sub_Admin_Username = '$_SESSION[subAdminUsername]'");
    	...
    	<select style = "width: 220px; height: 27px;" name="Reg_Number"  required="required">
            <option value="">Изберете регистрационен №</option>
                    <?php
                        ...
                        $query = "SELECT  Reg_Number FROM autos WHERE Individual_ID = '$Individual_ID' AND Sub_Admin_Username = '$_SESSION[subAdminUsername]'";
