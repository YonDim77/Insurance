...
$resultAutoServiceOFE = mysqli_query($con, "SELECT * FROM service WHERE Service_ID = $_SESSION[lastAutoFirmServiceID]
                                         AND Sub_Admin_Username = '".$subAdminUsername."'
...
if($_SESSION['lastAutoFirmServiceID'] > 0 && mysqli_num_rows($resultAutoService) > 0) {
...
<button id = "showTyresData" onclick = "showTyresData()" style = "border-radius: 2px; color: red; width: 31%;">Покажи запис на данни за гуми на МПС на физическо лице</button>
<button id = "hideTyresData" onclick = "hideTyresData()" style = "border-radius: 2px; color: red; width: 31%; display: none;">Скрий запис на данни за гуми на МПС на физическо лице</button>
<br>
<script>
...
//  $idCheck = mysqli_query($con2, "SELECT * FROM individual WHERE Individual_ID = $_POST[Individual_ID]");
  ...
  if(mysqli_num_rows($CheckDuplicateEntry) > 0 || !$idCheck || mysqli_num_rows($idCheck) < 1 || !mysqli_query($con2, $sqlInsertAutoRepair)) //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  {
    ...
    $message = "Възникна грешка!Дублиране на данни на МПС, прескачане ред на въвеждане на данни!";
...
if($lastAutoRepairID > 0 && mysqli_num_rows($resultAutoRepair) > 0) {
...
if($lastAutoTyresID > 0 && mysqli_num_rows($resultAutoTyres) > 0) {
