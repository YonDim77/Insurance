...
$result = mysqli_query($con, "SELECT * FROM autos WHERE AutosID = $lastAutoID AND Admin_Username = '".$adminUsername."'");
...
<h3 style = "margin-top: 7.0vw;">Запис на данни за застраховки, винетка и данък на МПС на физическо лице:</h3><br>
  <form name="input" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" accept-charset="UTF-8" style = ""> 
    <table class = "iDataInput">
      ...
      <br><br>
    <input type="submit" name="btnSaveAuto" value="Запис" style = "border-radius: 2px; color: red; width: 32%;">  
  </form> 
  <br><br>
  
<button id = "showServiceData" onclick = "showServiceData()" style = "border-radius: 2px; color: red; width: 32%;">Покажи запис на данни за сервиз на МПС на физическо лице</button>
<button id = "hideServiceData" onclick = "hideServiceData()" style = "border-radius: 2px; color: red; width: 32%; display: none;">Скрий запис на данни за сервиз на МПС на физическо лице</button>
<br>
<script>
    function showServiceData() {
        $(document.getElementById('autoServiceData')).toggle(1500);    //.style.display = 'block';
    ...
    function hideServiceData() {
        $(document.getElementById('autoServiceData')).toggle(1500);   //.style.display = 'none';
...
//  $idCheck = mysqli_query($con2, "SELECT * FROM individual WHERE Individual_ID = $_POST[Individual_ID]");
  ...
  VALUES
  ('$_POST[AutosID]', '$_POST[Individual_ID]', '$_POST[Admin_Username]', '$_POST[Sub_Admin_Username]', '$_POST[GTP_Date]', '$notSent', '$_POST[GTP_Sum]', '$_POST[GO_Date]', '$notSent',  
  ...
  if(!$idCheck || mysqli_num_rows($idCheck) < 1 || !mysqli_query($con2, $sqlInsertAuto))
  {
    ...
    $message = "Възникна грешка!Дублиране на ID/№ на МПС, грешно въведено ID/№ на физическо лице или грешка в йерархията подадминистратор потребител!";
...
if($lastAutoInsuranceID > 0) {
  ...
  echo "<table border='2' style = 'margin-top: 60px; margin-bottom: 30px;'>
...
?>
  ...
  <form id = "autoServiceData" name="input" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" accept-charset="UTF-8" style = "margin-top: 50px; display: none;">  
    <h3 style = "">Запис на данни за сервиз на МПС на физическо лице:</h3><br>
    <table class = "iDataInput">
        <tr>
            <!--<input type="text" name="Admin_Username" value = "<?php //echo $_SESSION['username']; ?>" style = "display: none;">-->
            <td>ID/№ на МПС*<br><input  type="number" name="AutosID" required="required" placeholder = "Задължително попълване"></td>
    ...
    <br><br>
    <input type="submit" name="btnSaveServiceAuto" value="Запис" style = "border-radius: 2px; color: red; width: 32%;">  
  </form>
  <br><br>
  
  <button onclick = "location.href='adminInputDataTyresAutoPerson.php'" style = "width: 32%; border-radius: 2px; color: red;">Въвеждане на данни за ремонти и гуми на МПС</button>
  <br><br>
...
//  $idCheck = mysqli_query($con2, "SELECT * FROM individual WHERE Individual_ID = $_POST[Individual_ID]");
  ...
  else
  {
    ...
    $message = "Данните са записани успешно!";
	echo "<script>alert('$message');</script>";
	echo "<script> location.replace('adminInputDataTyresAutoPerson.php'); </script>";
  }
  mysqli_query($con2, "SET AUTOCOMMIT=TRUE");
