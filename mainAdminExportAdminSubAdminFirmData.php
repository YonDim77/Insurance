...
<br>
<div align = "center">
    <h3 style = "margin-top: 7.0vw; text-align: center;">Данни за всички юридически лица, които са към <?php echo $_POST['Admin_Username']; echo $_POST['Sub_Admin_Username'];?></h3>
    <br><br>
    ...
    <form name="input" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" accept-charset="UTF-8" style = "color: black;">  
        ...
        Избери потребител<br><select style = "width: 174px; height: 27px;" name="Sub_Admin_Username">
            ...
            <br><br>
            <input type="submit" name="btnDisplayData" value="Покажи данни" style = "border-radius: 2px; color: red;"> 
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $btnDisplayData) {
    ...
    if((strlen($admin) == 0 && strlen($subAdmin) == 0) || (strlen($admin) > 0 && strlen($subAdmin) > 0)) {
        ...
        $result = mysqli_query($con, "SELECT * FROM legalentity WHERE Admin_Username = '$_POST[Admin_Username]' OR Sub_Admin_Username = '$_POST[Sub_Admin_Username]'");
        ...
        {
            $message = "Няма данни за юридически лица, които да са към ". $admin . "  ". $subAdmin . "!";
	    	echo "<script>alert('$message');</script>";
        }
